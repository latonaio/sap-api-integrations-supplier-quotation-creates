...
import (
	...
	"io/ioutil"
	"net/http"
	"sap-api-integrations-supplier-quotation-creates/SAP_API_Caller/requests"
	sap_api_output_formatter "sap-api-integrations-supplier-quotation-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callSupplierQuotationSrvAPIRequirementHeader("SupplierQuotation", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSupplierQuotationSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("SupplierQuotation('%s')/to_Item", item.SupplierQuotation)
	outputDataItem, err := c.callSupplierQuotationSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSupplierQuotationSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
